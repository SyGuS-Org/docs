...
In the following, we summarize how the base logic and extended
logic restrict the set of terms that may appear in constraints and grammars.
...
\section{Reference Grammars for SMT LIB Logics}
\label{sec:smt-theories}

In this section, for conveinence, we provide the concrete
grammar corresponding to the (most liberal) grammars corresponding
to all terms that belong to logics of interest.
Note this is not intended to be a complete list of all logics.

For each logic, we give a grammar for
Notice that all logics include the if-then-else term ${\tt ite}$
as well as the equality predicate ${\tt =}$.
...
Each grammar omits the predicate symbols that are shared by all logics
...
admits exactly the integer-typed terms in the logic of
...
(synth-fun f ((x Int)) Int
   ...
   ((y_term Int  (y_const 
    ...
    (y_pred Bool ((> y_term y_term)
                  ...
                  (<= y_term y_term))))))
\end{lstlisting}
The grammar for $g$ admits exactly the integer-typed terms in the logic of
...
The following grammar for $f$
admits exactly the real-typed terms in the logic of
