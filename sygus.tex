...
\title{The SyGuS Input Language Standard Version 2.0}
...
\item The syntax for the $\synthfunkwd$
...
is now the same one from SMT-LIB versions 2.0 and later.
There are two notable changes
with respect to the previous format as a result of this change.
First,
...
This means that certain new symbols are now present in the signature,
...
\item
A formal notion of features has been added to the background logic,
...
the set of commands that are allowable in an input.
...
This treatment of each of these literals coincides with
...
Numerals are written as usual,
either the digit $0$,
or a non-empty sequence of digits $\left[{\tt 0}-{\tt 9}\right]$
that does not begin with $0$.

\paragraph{Decimals ($\realconst$)}
Real numbers are written using their decimal expansion.
Their syntax is $\intconst{\tt .\kstar{0}}\intconst$.

\paragraph{Booleans ($\boolconst$)}
Symbols $\truekwd$ and $\falsekwd$ denote the Boolean constants true and false.

\paragraph{Hexidecimals ($\hexconst$)}
Hexidecimals are written in their
traditional representation, with ${\tt \#x}$
...
\paragraph{Binaries ($\hexconst$)}
Binaries are written in their
traditional representation, with ${\tt \#b}$
...
\begin{alignat*}{1}
 & \begin{array}{rcl}
 \feature & ::= & \fgrammarskwd\mbox{ }|\mbox{ }\ffwddeclskwd\mbox{ }|\mbox{ }\frecursionkwd\mbox{ }|\mbox{ }\fquantifierskwd
\end{array}
\end{alignat*}
...
This sets the base logic component of the SyGuS background logic
...
We do not standardize any concrete examples of feature sets
in this document.
...
If provided, the grammar $G$ must also be one that is allowed by the SyGuS background logic.
...
state that correspond to an invariant synthesis problem for $S$,
...
Notice that we require that $f_1, \ldots, f_n$ be
provided in the order in which they were declared.
...
and are generally independent of the base logic.
\end{enumerate}
...
The following
two subsections gives an overview of the kinds of base logics that
...
\subsection{Base Logics and Theories}
...
Similar to the treatment of default logics in SMT-LIB,
the base logic component of the default SyGuS logic includes only the
\emph{core theory}.
...
The logics mentioned in this section supplement the signature of the
core theory with sorts and symbols.
...
Logics that include datatypes permit
$\dtsdeclkwd$ commands, whereas all others do not.
The signature used in inputs where the theory of datatypes is enabled
is largely determined by the concrete datatypes definitions
which append datatype sorts, 
...
These declarations are made via SMT-LIB commands 
...
Other features may be given that are solver specific, which are not covered
here.
...
a term $t$ is allowed in this context
...
A grammar $G$ is not allowed by the base logic if
...
unless the feature $\ffwddeclskwd$ is enabled;
it is not allowed if $t$ 
contains $f$ itself unless the feature $\frecursionkwd$ is enabled;
it is not allowed regardless of the terms it generates unless $\fgrammarskwd$ is enabled.
...
for what constitutes a correct solution for synthesis conjecture.
...
term allowable in the background logic.
\end{enumerate}
%\rem{More}
...
%\end{def}
The formal definition for $T$-valid
...
In this section, for convenience, we provide the concrete
...
In particular, we focus on logics that consist of a single theory
...
Notice that since the logic specified above in ${\tt S}$,
meaning that certain functions involving the integer sort like
${\tt str.len}$ are included in its signature.
...
\subsection{Parametric Datatype Definitions}

\subsection{Recursive Functions}

\subsection{Match Terms}

\subsection{Attribute Annotations}
