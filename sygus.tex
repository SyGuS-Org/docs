...
This means that certain symbols have been added to the signature,
...
${\tt str.to-int}$ and ${\tt str.from-int}$.
...
we formalize what consistutes correct response to a SyGuS input.
...
on the kinds of SyGuS commands that are allowable in an input.
...
This set the feature set component of the SyGuS background logic
to the one specified by $S$.
...
Setting a feature set is syntax sugar
...
If provided, the grammar $G$ must also be one that is allowable by the SyGuS background logic.
...
then any term may be given in the body of a solution for $S$
...
The base logic component of the default SyGuS logic is the empty logic,
