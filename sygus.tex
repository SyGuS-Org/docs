...
version of the logic $S$, indicating that quantified formulas
cannot appear in constraints;
...
The following
two subsections gives an overview of some of the logics that
are defined in SMT-LIB, and the feature sets
standardized by this document.
The next two subsections 
then summarize how these
...
\subsection{Input/Output Logics and Theories}
...
These logics may be provided either as an input logic or an output logic.
...
Assuming our input logic is a standard SMT-LIB background logic,
a term $t$ is allowed to be a subterm within a $\constraintkwd$ command
if it is allowed by that logic, as prescribed by the SMT-LIB standard.
...
it generates some term $t$ that does not belong to the output logic.
%For instance, 
...
term allowed in the background logic.
\end{enumerate}
%\rem{More}
...
\subsection{The Theory of Mixed Integers and Reals}
