...
This adds the set of sort and function symbols
from the logic specified by $S$ to the current signature.
This command is not well-formed if $S$ is not
one of the logics allowe

For examples of common SMT theories, see Section~\ref{sec:smt-theories}.
...
In other words, $G_{T,\vec{x},\sigma}$ is a grammar that
generates all well-sorted terms of sort $\sigma$
whose symbols belong to the signature of $T$ and whose
free variables are in $x_1, \ldots, x_n$.
