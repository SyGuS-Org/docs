...
which is used for further refining restrictions
on the kinds of constraints and grammars that may appear in an input.
...
unless feature $\efwddeclskwd$ is enabled;
it is not allowed by the extended logic if $t$ 
contains $f$ itself unless feature $\erecursionkwd$ is enabled.
