...
see Appendix~\ref{sec:smt-logic}.
...
Sorts may be defined by theories (see examples in Appendix~\ref{sec:smt-logics})
...
Examples of the latter are given in Appendix~\ref{sec:smt-logics}.
...
a \emph{base logic} and an \emph{extended logic},
...
This adds the extension specified by $S$
to the extended logic of the background logic.
This may correspond enabling or disabling a feature of the language 
or a modification to the current logic.
All extensions standardized by this document 
...
The background logic of SyGuS consists of two parts:
\begin{enumerate}
...
\item An \emph{extended logic}, which augments the background
logic with additional restrictions that are not expressible in a base logic
and are parametric to the base logic.
\end{enumerate}
...
an extended logic can be seen as a set of values
taken from the following enumeration.
\[
\equantifierskwd\mbox{ }|\mbox{ }\efwddeclskwd\mbox{ }|\mbox{ }\erecursionkwd
\]
We refer to the above values as \emph{flags} of the extended logic,
and say that a flag is enabled in the extended logic if it contains that flag.
The above flags indicate (respectively) whether
...
by the $\setextensionkwd$ command.

In the following, 
we first give an overview of the kinds of base logics that
are defined in SMT-LIB.
We then summarize how the base logic and extended
logic restrict the set of terms that may appear 
in constraints and grammars in SyGuS inputs.
...
\subsection{Base Logics in the SMT-LIB standard}
\label{sec:smt-logic}

SMT-LIB provides a catalog of standard logics,
available at \url{www.smt-lib.org}.
We briefly review these in the following.



\subsection{Logical Restrictions on Constraints}
...
Exceptions to this rule are described in this section.
...
formulas with quantifierstheory
unless the flag $\equantifierskwd$ is enabled in the extended logic.

%\rem{Reference extension?}

\subsection{Logical Restrictions on Grammars}
...
The extended logic may impose additional restrictions
...
A grammar for function-to-synthesize $f$ is not allowed by the extended logic
if it contains a rule whose conclusion is a term $t$
whose expanded form contains applications of functions-to-synthesize
unless $\efwddeclskwd$ is enabled in the extended logic;
it is not allowed by the extended logic if $t$ 
contains $f$ itself unless $\erecursionkwd$ is enabled in the extended logic.
...
available at \url{www.smt-lib.org}.
More details on the theory and logic declarations
can be found in Sections 3.7 and 3.8
of the SMT-LIB version 2.6 standard~\cite{}.

For each grammar, we omit some 
of the predicate symbols that are shared by all logics
according to the SMT-LIB standard. This includes:
\begin{enumerate}
\item A binary equality predicate
${\tt =}$ of sort $\sigma \times \sigma \rightarrow \sbool$
for each sort $\sigma$,
%\item An if-then-else term ${\tt ite}$
\item All Boolean connectives, including 
${\tt and}$, ${\tt or}$, ${\tt =>}$, ${\tt xor}$.
\end{enumerate}
We provide the grammar
for a single function $f$ over one of the sorts in the logic,
...
(synth-fun f ((x Int)) Int
   ((y_term Int) (y_const Int) (y_pred Bool))
   ((y_term Int  (y_const 
                  (Variable Int)
                  (- y_term)
                  (+ y_term y_term)
    ...
    (y_pred Bool ((> y_term y_term)
                  ...
                  (<= y_term y_term))))))
\end{lstlisting}
The grammar for $g$ generates exactly the integer-typed terms in the logic of
...
(synth-fun f ((x Real)) Real
   ((y_term Real) (y_const Real) (y_pred Bool))
   ((y_term Real (y_const 
                  (Variable Real) 
                  (- y_term)
                  (+ y_term y_term)
...
We further omit
the functions ${\tt (_ extract n m)}$ and ${\tt concat}$,
since they are polymorphic.
We instead give examples of these operators in Section~\ref{sec:examples}.

\begin{lstlisting}[basicstyle={\ttfamily}]
(set-logic BV)
(synth-fun f ((x1 (_ BitVec 32)) (x2 (_ BitVec 32))) Real
   ((y_term (_ BitVec 32)) (y_pred Bool))
   ((y_term (_ BitVec 32) (x1 x2 
                           (Constant (_ BitVec 32))
                           (ite yp y_term y_term)
                           ...
                           ))
    (y_pred Bool (...
                  )))))
