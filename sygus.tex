...
are associated with sorts $\sortexpr$, constructed via the following syntax.
...
\end{array}
\end{alignat*}
Notice that identifiers that comprise terms
...
Like sorts, the (qualified) identifiers that comprise terms
...
followed by a \emph{grouped rule list}
...
\rem{Logic e.g. Non-linear vs linear: terms generated by the grammars must be linear}
...
It defines $S$ as a term whose semantics are given by 
...
a \emph{grouped rule list} of the form
\[
