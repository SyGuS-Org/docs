...
The grammar must also be one that is allowable by $\mathcal{L}$.
For more details on the restrictions imposed on grammars by the logic,
see Section~\ref{sec:sygus-logic}.
If this command does not meet the restrictions
...
A grammar $G$ is not allowable by the logic
This command does not meet the restrictions
it generates some term $t$ that does not belong to the background logic.
For instance, 
...
$( \lambda X_1.\, t_1, \ldots, \lambda X_1.\, t_1 )$
