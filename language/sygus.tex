...
to be expressed in terms of new base logics whenever it is possible to do so.
On the other hand,
...
by this document that are \emph{not} covered by the SMT-LIB standard.
...
Given an SMT-LIB standard logic $X$,
the logic ${\tt INV\_}X$ denotes the SyGuS logic
...
Note that all well-formed inputs that are limited to
a single $\synthinvkwd$ command, followed by a single $\constraintinvkwd$
command meet the requirements of the input logic ${\tt INV\_}X$.
