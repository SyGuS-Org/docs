...
\noindent
\begin{equation*}
\mathit{\{setter\ commands\}^* \big(\{other\ commands\}^* \{check\ synth\ command\}?\big)^*}
\end{equation*}
where the set $\mathit{\{setter\ commands\}}$ includes
the \texttt{set-logic}, \texttt{set-feature} and \texttt{set-option}
commands, the set  $\mathit{\{other\ commands\}}$ include all the SyGuS
commands except the \texttt{check-synth} command and the commands in the
set $\mathit{\{setter\ commands\}}$ and the singleton set $\mathit{\{check\
synth\ command\}}$ consists of the \texttt{check-synth} command.

In other words, a SyGuS input is well formed if it begins with a block of
zero or more \texttt{set-logic}, \texttt{set-feature}
and \texttt{set-option} commands, followed by zero or more instances of a
block of commands, where each block consists of commands terminated by an
optional \texttt{check-synth} command.
...
\subsubsection{Additional Logics Defined by the SyGuS Standard}
