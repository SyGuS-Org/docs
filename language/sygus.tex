...
bit-width 32 and 16.
In that grammar, ${\tt concat}$ denotes
the operator that concatenates its two arguments,
and the indexed operator ${\tt (\_\ extract\ n\ m)}$
denotes the the operator that returns a bit-vector containing bits $n$ through $m$ of its
argument, where $n \geq m$.
The semantics of both of these operators is defined in the SMT-LIB standard.
