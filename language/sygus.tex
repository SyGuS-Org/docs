...
Details on objectives in given in \cref{sec:objectives}.
...
For details, see \cref{sec:objectives}.
...
to this command.
For details on the default semantics for objectives, see \cref{sec:objectives}.
...
(optimize-synth ((! (f 0) :max) (! (f 100) :max)) :lexicographic)
\end{lstlisting}
...
\end{lstlisting}
The first example is more preferred, since the first value in the first
