...
for solvers targeting the syntax-guided synthesis problem and users of these solvers.
...
with those in the SMT-LIB version 2.6 format~\cite{BarFT-RR-17}.
...
\end{array}
\end{alignat*}
More details on features are given in Section~\ref{ssec:feature-sets}.
...
\end{equation*}
where the set $\mathit{\{check\ logic\ command\}}$ consists
...
The logic string $S$ can be a standard one defined in SMT-LIB~\cite{BarFT-RR-17}
...
An example demonstrating this case is given in Section~\ref{sec:examples}.
...
equalities between pairs of terms $f( \vec c )$ and $d$,
where $f$ is a function
and $\vec c$, $d$ are constants.
We refer to an equality of the aforementioned form as a \emph{PBE equality},
which denotes a relationship between the inputs and output of 
function $f$ for a single point.
Notice that this logic is a subset of ${\tt QF\_}X$.

We overload this logic string and use it for referring to a SyGuS logic as well.
...
By construction of the overall synthesis conjecture, SyGus command sequences 
that meet the requirements of the input logic ${\tt PBE\_}X$
require that each ${\tt constraint}$ command takes as argument a PBE equality.

\paragraph{Single Invariant-to-Synthesize (INV)}
Given an SMT-LIB standard logic $X$,
the base logic ${\tt INV\_}X$ denotes the logic
where constraints are limited to the invariant synthesis
problem for a single invariant-to-synthesize.
Concretely,
this means that constraints allowed in its input logic are limited to those that 
are (syntactically) a conjunction of three implications, 
for a single invariant to synthesize $I$:
\begin{enumerate}
\item The first is an implication whose antecedant is an arbitrary formula
in the logic (the pre-condition) and
and whose conclusion is
an application $I( \vec x )$ where $\vec x$ are variables,

\item The second is an implication whose antecedant is
$I( \vec x ) \wedge \varphi$
where $\vec x$ are variables and $\varphi$ is an arbitrary formula
in the logic (the transition relation),
and whose conclusion is $I( \vec y )$
where $\vec y$ are variables disjoint from $\vec x$,

\item The third is an implication whose antecedant is 
an application $I( \vec x )$ where $\vec x$ are variables,
and whose conclusion is an arbitrary formula in the logic (the post-condition).
\end{enumerate}

Like the previous section, 
we overload this logic string and use it for referring to SyGuS logics as well.
Logic ${\tt INV\_}X$ denotes the SyGuS logic
whose input logic is ${\tt INV\_}X$,
