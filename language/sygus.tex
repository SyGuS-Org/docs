...
The attributes $A_1 \ldots A_n$ can be used to indicate properties of this weight.
...
\item $\paren{\optsynthkwd\mbox{ }\paren{t_1 \ldots t_j}\mbox{ } A_1 \ldots A_k}$
...
specified by the list of terms $t_1, \ldots, t_j$ and attributes $A_1, \ldots, A_k$.
Objectives can be specified both using annotations on terms $t_1, \ldots, t_j$
as well as the use of attributes $A_1, \ldots, A_k$ to specify orderings on tuples.
Details on objectives in given in Section~\ref{sec:objectives}.
\end{itemize}
...
\item
A (successful) well-formed response to $\paren{\optsynthkwd\mbox{ }\paren{t_1 \ldots t_j}\mbox{ } A_1 \ldots A_k}$
...
\]
To be a well-formed response, 
$c_1, \ldots, c_j$ must be values of the same type as $t_1, \ldots, t_j$,
...
are consistent valuations of terms $t_1, \ldots, t_j$
...
queries of the form:
\[
\paren{\optsynthkwd\mbox{ }\paren{t_1 \ldots t_j}\mbox{ } A_1 \ldots A_k}
\]
Recall from \cref{sec:output} that a successful response to this command
contains:
\begin{enumerate}
\item A tuple of terms $(c_1, \ldots, c_j)$ corresponding to the valuation of terms $(t_1, \ldots, t_j)$, and
...
\[
\Psi \wedge \bigwedge_{i=1}^{j} t_j \teq c_j
\]
...
for the synthesis conjecture $\exists f_1, \ldots, f_n. \Psi_{o}$.
...
In this section, we standardize several common use cases of the $\optsynthkwd$ command.
...
An optimization query is made to minimize the number of branches in $\texttt{f}$.
...
and hence it is more preferred based on the specified objective.
\end{example}
