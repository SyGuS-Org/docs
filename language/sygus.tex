...
\title{The SyGuS Language Standard Version 3.0}
...
\begin{enumerate}
\item Assumptions
\item Oracle Functions are introduced. These are functional symbols whose interpretation is associated to an external oracle. The functions are treated as universally quantified uninterpreted functions, and calls to that external oracle place with concrete input values generate assumptions over this behaviour.
\item Oracle constraints are introduced. Oracle constraints are constraints that are generated when an external oracle is called with a concrete set of input values. The constraints conjoined to the semantic correctnes specification. 
\item A set of new commands are introduced to generate specific types of oracles, for instance input-output oracles.
\end{enumerate}
...
\begin{alignat*}{1}
 & \exists f_1,\ldots,f_n.\, \forall x_1,\ldots,x_m.\,(\alpha_1 \wedge \ldots \wedge \alpha_r) \implies (\varphi_1 \wedge \ldots \wedge \varphi_p )[f_1,\ldots,f_n,x_1,\ldots,x_m]
\end{alignat*}
...
for functions-to-synthesize $( f_1, \ldots, f_n )$
in conjecture $\exists f_1, \ldots, f_n.\,\forall v_1, \ldots,
v_m. \alpha \implies \varphi$ in background theory $T$
if $\forall v_1, \ldots,
v_m.\, \alpha \implies \varphi$ is $T$-valid formula
