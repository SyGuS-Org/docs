...
An oracle can be queried either via command line (default) 
or using text files with the extension
\textit{.oracle}. If the oracle should be queried via text files, it is declared with the attribute $\texttt{:file}$.
...
When an oracle that is queried by command line is executed with the command
${\tt oracle \,\, \paren{v_1 \ldots v_n}}$,
or an oracle that is queried via text files is executed with the command
${\tt oracle \,\,filename.oracle}$, and and \textit{filename.oracle} contains the string
$\paren{v_1 \ldots v_n}$, 
...
specifies that an oracle should be queried via text files, as described in Section~\ref{sec:oracleimplementations}
\end{enumerate}
