...
in particular,
the response ${\tt (fail)}$ from the previous standard
should now be given as ${\tt fail}$.
Additionally, a solver may answer ${\tt infeasible}$, indicating
that it is certain there are no solutions.
...
\item
The output ${\tt fail}$,
indicating that the solver failed to solve the conjecture.
...
We do not define the correctness of a ${\tt infeasible}$ response 
in this document. 
...
Otherwise, the response ${\tt fail}$ indicates that the solver
...
\begin{enumerate}
\item The first is an implication whose antecedant (the pre-condition) 
...
\begin{example}[Programming by Examples (PBE)]
Consider the following example:
\begin{lstlisting}[basicstyle={\ttfamily}]
(set-logic PBE_SLIA)
...
Since this example contains only linear arithmetic terms,
a single function-to-synthesize
