...
\begin{definition}
An oracle queried via command line \emph{implements an interface} with input
...
\begin{definition}
An oracle queried via text files \emph{implements an interface} with input
$(\sigma_1 \times \ldots \times \sigma_n)$ and output
$(\tau_1 \times \ldots \times \tau_m)$ if
it has the following behavior.
...
specifies that an oracle should be queried via command line, as described in \cref{sec:oracleimplementations}.
\end{enumerate}
