...
the base logic ${\tt PBE\_}X$ denotes the SyGuS logic
where the formulas in the \emph{input logic} are limited to conjunctions
of equalities between pairs of terms $f( \vec c )$ and $d$,
where $f$ is a function-to-synthesize,
and $\vec c$, $d$ are constants.
Notice that this logic is a subset of ${\tt QF\_}X$.

The same logic string is used for referring to a SyGuS logic.
...
In other words, the constraints of this logic
are limited to be in PBE form, 
...
Given an SMT-LIB standard logic $X$,
the base logic ${\tt INV\_}X$ denotes the SyGuS logic
where the constraints in the \emph{input logic} are limited to an
invariant synthesis problem for a single invariant-to-synthesize.
Concretely,
this means that formulas in this logic are limited to those that 
...
\end{enumerate}
Note that all well-formed inputs that are limited to a sequence
of two commands, a $\synthinvkwd$ command followed by a $\constraintinvkwd$ command,
meet the requirements of the input logic ${\tt INV\_}X$.
